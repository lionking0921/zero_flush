...
DEFINE_bool(zeroflush, false,
            "If true, use ZeroFlush mode: memtable stores only SlimLocator, "
            "value is persisted in partitioned WAL only. Must be used with "
            "num_column_families=1 (default).");
...
DEFINE_double(zf_base_merge_min_ratio, -1.0,
             "ZeroFlush: base merge trigger ratio (sealed/overlap); "
             "-1 = ZeroFlushOptions default (0.25)");
DEFINE_int32(zf_max_pending_epochs, -1,
            "ZeroFlush: max pending (unmaterialized) epochs for write "
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 private:
  ...
  void OpenDb(Options options, ToolHooks& hooks, const std::string& db_name,
    ...
    if (FLAGS_num_column_families > 1) {
    ...
    } else if (FLAGS_zeroflush) {
      // ZeroFlush 模式：替换 memtable_factory 为 SlimMemTableRepFactory，
      ...
      if (FLAGS_zf_max_pending_epochs > 0) {
        zfo.max_pending_epochs =
            static_cast<uint64_t>(FLAGS_zf_max_pending_epochs);
      }
      if (FLAGS_zf_base_merge_min_ratio >= 0.0) {
        zfo.base_merge_min_ratio = FLAGS_zf_base_merge_min_ratio;
      }
