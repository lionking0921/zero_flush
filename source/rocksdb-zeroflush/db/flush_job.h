...
class FlushJob {
 ...
 private:
  ...
  // ---- ZeroFlush M3.2：物化全部 mems_ 对应的 epoch（§6.4 分派）----
  // 前提：PickMemTable 判定为 ZF 模式（mems_[0] 关联 epoch）后由 Run() 调用。
  // 锁语义与 WriteLevel0Table 一致：进入时持锁，物化期释放，返回前重取锁。
  // 内部逐 epoch 建 ZfMaterializeJob：K 路并行读封存 WAL → 定层 → 输出并入
  // zf_batch_outputs_；调用方在持锁下把批次输出单次写入 edit_（mems_[0]），
  // 由 TryInstallMemtableFlushResults 以单次 LogAndApply 原子安装（§6.3）。
  Status ZfMaterializeAllEpochs();
  ...
  // ---- ZeroFlush M3.2 ----
  // 本批次是否为 ZF 物化模式（PickMemTable 时判定：mems_[0] 关联封存 epoch）。
  bool zf_mode_ = false;
