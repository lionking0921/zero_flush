...
WriteStallCondition ColumnFamilyData::RecalculateWriteStallConditions(
  ...
  if (current_ != nullptr) {
    ...
    ROCKS_LOG_DEBUG(ioptions().info_log.get(),
                    "Recalc: imm_num=%d max_wbuf=%d l0=%d hard_pcb=%lu "
                    "soft_pcb=%lu cf=%s cond=%d cause=%d",
                    imm()->NumNotFlushed(),
                    mutable_cf_options.max_write_buffer_number,
                    vstorage->l0_delay_trigger_count(),
                    (unsigned long)mutable_cf_options
                        .hard_pending_compaction_bytes_limit,
                    (unsigned long)mutable_cf_options
                        .soft_pending_compaction_bytes_limit,
                    name_.c_str(), (int)write_stall_condition,
                    (int)write_stall_cause);
...
void ColumnFamilyData::CreateNewMemtable(SequenceNumber earliest_seq) {
  ...
  mem_->Ref();
  // ---- ZeroFlush：把 cfd 持有的 zf_ctx 传播到新 MemTable ----
  // 否则 SwitchMemtable 后读路径会把 16B SlimLocator 当 value 返回。
  if (zf_ctx_ != nullptr) {
    mem_->SetZeroFlushContext(zf_ctx_);
  }
}
