...
class ReadOnlyMemTable {
 public:
  ...
  // ZeroFlush：该 memtable 关联的封存 epoch（0 = 非 ZF 写路径）。
  // 基类默认 0；仅 ZF 路径的 MemTable 覆写。flush 以非 0 判定走物化分派。
  virtual uint64_t GetZfEpoch() const { return 0; }
...
class MemTable final : public ReadOnlyMemTable {
 public:
  ...
  // ---- ZeroFlush：Slim 模式上下文（nullptr = 原生模式）----
  // 写路径（leader）与读路径（SaveValue / MemTableIterator）据此把
  // memtable 条目中的 SlimLocator 解析为分区 WAL 中的 value。
  void SetZeroFlushContext(
      const std::shared_ptr<zeroflush::ZeroFlushContext>& ctx) {
    zf_ctx_ = ctx;
  }
  const std::shared_ptr<zeroflush::ZeroFlushContext>& GetZeroFlushContext()
      const {
    return zf_ctx_;
  }

  // ---- ZeroFlush M2：本 MemTable 关联的 epoch（封存时的代际号）----
  // SealEpochAndSwitch 切换前设置；~MemTable 调用 ctx->ReleaseEpoch 释放
  // SealedFileCache 引用。0 表示未关联（活跃 mutable）。
  void SetZfEpoch(uint64_t epoch) { zf_epoch_ = epoch; }
  uint64_t GetZfEpoch() const override { return zf_epoch_; }
 ...
 private:
  ...
  // ---- ZeroFlush：Slim 模式上下文（nullptr = 原生模式）----
  // ---- ZeroFlush M2.1：本 mem 关联的封存 epoch（0=无）----
  uint64_t zf_epoch_ = 0;
  std::shared_ptr<zeroflush::ZeroFlushContext> zf_ctx_;
};
