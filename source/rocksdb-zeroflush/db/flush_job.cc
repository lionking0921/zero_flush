...
void FlushJob::PickMemTable() {
  ...
  // ZeroFlush M3.2：mems_[0] 关联封存 epoch 且 cfd 挂有 zf_ctx 时走 ZF 物化
  // 路径（K 路并行读封存 WAL + 层级下探直装），否则维持原生 flush。
  zf_mode_ = (mems_[0]->GetZfEpoch() != 0 && cfd_->GetZfCtx() != nullptr);
...
Status FlushJob::Run(LogsWithPrepTracker* prep_tracker, FileMetaData* file_meta,
  ...
  if (mempurge_s.ok()) {
    ...
    ROCKS_LOG_DEBUG(db_options_.info_log, "[%s] [JOB %d] calling WriteLevel0Table",
                    cfd_->GetName().c_str(), job_context_->job_id);
    // This will release and re-acquire the mutex.
    if (zf_mode_) {
      // ZeroFlush M3.2：ZF 物化路径（K 路并行 + 层级下探直装，§6）。
      s = ZfMaterializeAllEpochs();
    } else {
      s = WriteLevel0Table();
    }
    ROCKS_LOG_DEBUG(db_options_.info_log, "[%s] [JOB %d] WriteLevel0Table returned %s",
                    cfd_->GetName().c_str(), job_context_->job_id,
                    s.ToString().c_str());
  }
  ...
  if (!s.ok()) {
    if (zf_mode_) {
      // ZeroFlush M3.2：清理本批次已生成的 SST（物化失败或 manifest 安装
      // 失败）。ZfMaterializeJob 已清理自身产出；这里处理批次中前面 epoch
      // 的成功输出，并逐文件释放 TableCache 条目。
      for (auto& o : zf_batch_outputs_) {
        if (o.meta.fd.GetFileSize() > 0) {
          TableCache::ReleaseObsolete(cfd_->table_cache()->get_cache().get(),
                                      o.meta.fd.GetNumber(), nullptr /*handle*/,
                                      mutable_cf_options_.uncache_aggressiveness);
        }
        const std::string fname = TableFileName(
            cfd_->ioptions().cf_paths, o.meta.fd.GetNumber(),
            o.meta.fd.GetPathId());
        db_options_.env->DeleteFile(fname).PermitUncheckedError();
      }
      zf_batch_outputs_.clear();
    }
                                             ...
                                             1 /* count**/);
  {
...

    {
      ...
      ROCKS_LOG_DEBUG(db_options_.info_log,
                      "[%s] [JOB %d] BuildTable returned %s meta_size=%" PRIu64,
                      cfd_->GetName().c_str(), job_context_->job_id,
                      s.ToString().c_str(), meta_.fd.GetFileSize());
...
Status FlushJob::ZfMaterializeAllEpochs() {
  // 锁语义与 WriteLevel0Table 一致：进入持锁，物化期释放，返回前重取。
  db_mutex_->AssertHeld();
  assert(!mems_.empty());
  assert(zf_mode_);
  ...
  // 组装 ZfMaterializeCtx（引用 FlushJob 成员；生命周期限于本调用）。
  zeroflush::ZfMaterializeCtx mc;
  mc.dbname = dbname_;
  mc.cfd = cfd_;
  mc.db_options = &db_options_;
  mc.mutable_cf_options = &mutable_cf_options_;
  mc.file_options = &file_options_;
  mc.versions = versions_;
  mc.job_context = job_context_;
  mc.log_buffer = log_buffer_;
  mc.output_compression = output_compression_;
  mc.io_priority = GetRateLimiterPriority();
  mc.stats = stats_;
  mc.event_logger = event_logger_;
  mc.thread_pri = thread_pri_;
  mc.io_tracer = io_tracer_;
  mc.db_id = db_id_;
  mc.db_session_id = db_session_id_;
  mc.job_id = job_context_->job_id;
  mc.earliest_snapshot = earliest_snapshot_;
  mc.seqno_to_time_mapping = seqno_to_time_mapping_;
  mc.full_history_ts_low = full_history_ts_low_;
  mc.blob_callback = blob_callback_;
  mc.fast_sst_open = fast_sst_open_;
  mc.db_mutex = db_mutex_;
  // M3.3：融合归并的基础版本与 CompactionPicker（ZfMaterializeJob 阶段 0
  ...
  for (auto& o : zf_batch_outputs_) {
    // M3.3 融合归并：跳过被批内后序输出替代的文件（不安装；其物理
    // 文件由替换者的 Run() 删除）；其余先删除被替换的 base 输入文件
    // （existing，文件号即已提交版本中的 base 文件），再添加输出。
    if (o.superseded) {
      continue;
    }
  ...
  // Piggyback FlushJobInfo on the first flushed memtable（对齐原生路径）。
  mems_[0]->SetFlushJobInfo(GetFlushJobInfo());

  ROCKS_LOG_INFO(db_options_.info_log,
                 "[%s] [JOB %d] ZeroFlush materialized %zu epochs (%zu files, "
                 "%" PRIu64 " bytes) in %" PRIu64 " us",
