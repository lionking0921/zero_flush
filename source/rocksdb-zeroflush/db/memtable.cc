...
MemTable::~MemTable() {
  ...
  // ---- ZeroFlush M2.1：释放本 mem 持有的封存 epoch 引用 ----
  // MemTable 销毁意味着所有引用（包括 SuperVersion/迭代器）已释放，
  // 此时再 ReleaseEpoch 是安全的。SealedFileCache 内 refcount-- 后
  // 若归零，文件移入 pending_unlink 等待 PurgeSealedFiles 真正删除。
  if (zf_ctx_ != nullptr && zf_epoch_ != 0) {
    zf_ctx_->ReleaseEpoch(zf_epoch_);
    zf_epoch_ = 0;  // 幂等保护
  }
  assert(refs_ == 0);
}
...
class MemTableIterator : public InternalIterator {
 public:
  ...
  MemTableIterator(
      ...
      : bloom_(nullptr),
        ...
        zf_ctx_(mem.GetZeroFlushContext().get()),
        valid_(false),
        value_pinned_(
            zf_ctx_ == nullptr &&
  ...
  Slice value() const override {
    ...
    if (zf_ctx_ != nullptr) {
      // ZeroFlush Slim 模式：条目中存放 16B SlimLocator，value 的唯一
      // 持久副本在分区 WAL 中，定点读取。zf_value_buf_ 为 mutable：
      // arena 模式下迭代器析构不被调用，缓冲堆内存由 Arena 存续期
      // 托管（M1 已知限制，M2 换 arena 可增长 buffer）。
      const Slice loc =
          GetLengthPrefixedSlice(key_slice.data() + key_slice.size());
      Status s = zf_ctx_->ReadValue(loc, &zf_value_buf_);
      if (!s.ok()) {
        // 定点读失败视为条目损坏：返回空 value（M1 由 CRC 保证完整性）。
        zf_value_buf_.clear();
      }
      return Slice(zf_value_buf_);
    }
 ...
 private:
  ...
  // ZeroFlush Slim 模式上下文（nullptr = 原生）与 value 解析缓冲。
  const zeroflush::ZeroFlushContext* zf_ctx_;
  mutable std::string zf_value_buf_;
  bool valid_;
  bool value_pinned_;
  bool arena_mode_;
...
struct Saver {
  ...
  bool allow_data_in_errors;
  uint32_t protection_bytes_per_key;
  // ZeroFlush Slim 模式上下文（nullptr = 原生）；SaveValue 据此把条目中的
  // SlimLocator 解析为分区 WAL 中的 value。
  zeroflush::ZeroFlushContext* zf_ctx = nullptr;
      ...
      case kTypeValuePreferredSeqno: {
        ...
        if (s->zf_ctx != nullptr) {
          ...
          if (type == kTypeValuePreferredSeqno) {
            *(s->status) = Status::NotSupported(
                "ZeroFlush: kTypeValuePreferredSeqno not supported");
            *(s->found_final_value) = true;
            return false;
          }
          const Slice loc = GetLengthPrefixedSlice(key_ptr + key_length);
          *(s->status) = s->zf_ctx->ReadValue(loc, &zf_tmp);
          if (!s->status->ok()) {
            *(s->found_final_value) = true;
            return false;
          }
...
void MemTable::GetFromTable(const LookupKey& key,
  ...
  saver.zf_ctx = GetZeroFlushContext().get();
