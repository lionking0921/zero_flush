...
Status DBImpl::CloseHelper() {
  ...
  mutex_.Lock();
  ROCKS_LOG_INFO(immutable_db_options_.info_log,
                 "CloseHelper: acquired mutex, bg_flush_sched=%d "
                 "bg_compact_sched=%d bg_purge_sched=%d",
  ...
  while (bg_bottom_compaction_scheduled_ || bg_compaction_scheduled_ ||
    ...
    ROCKS_LOG_DEBUG(immutable_db_options_.info_log,
                    "CloseHelper: waiting for BG work, bg_flush=%d "
                    "bg_compact=%d bg_purge=%d",
                    bg_flush_scheduled_, bg_compaction_scheduled_,
                    bg_purge_scheduled_);
...
bool DBImpl::GetProperty(ColumnFamilyHandle* column_family,
  ...
  const DBPropertyInfo* property_info = GetPropertyInfo(property);
  value->clear();
  // M3.0：ZeroFlush 指标（rocksdb.zeroflush.*）不经 DBPropertyInfo 表，
  // 直接分派给 zf_ctx_（M3_DESIGN.md §13）。
  if (zf_ctx_ != nullptr && property.starts_with("rocksdb.zeroflush.")) {
    return zf_ctx_->GetProperty(property.ToString(), value);
  }
  auto cfd =
