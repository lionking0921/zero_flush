...
class DBImpl : public DB {
 public:
  ...
  void SetZeroFlushContext(
    ...
    zf_ctx_ = ctx;
    // 同时注入 default CF 的 MemTable（写路径与读路径共用）
    ColumnFamilyData* cfd = GetDefaultColumnFamily();
    if (cfd != nullptr) {
      // cfd 也持有 ctx，使 SwitchMemtable 产生的新 MemTable 能继承
      // （详见 column_family.cc CreateNewMemtable）。
      cfd->SetZfCtx(ctx);
      if (cfd->mem() != nullptr) {
        cfd->mem()->SetZeroFlushContext(ctx);
      }
    }
  }
  ...
  ColumnFamilyData* GetDefaultColumnFamily() const {
    return versions_->GetColumnFamilySet()->GetDefault();
  }
  void SetZeroFlushLastSequence(SequenceNumber seq) {
    versions_->SetLastSequence(seq);
  }

  // ---- ZeroFlush M2.1：封存 + 切表 ----
  // 由 ZeroFlushContext::SealEpochAndSwitch 调用。封装 SwitchMemtable 调用
  // （该函数为 private，不能从 ctx 直接调用）。调用方必须持 DB mutex 且
  // 是 write thread leader。
  Status ZfSwitchMemtable(ColumnFamilyData* cfd);
