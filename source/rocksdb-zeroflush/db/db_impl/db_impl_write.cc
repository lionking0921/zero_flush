...
Status DBImpl::WriteImpl(
  ...
  if (zf_ctx_ != nullptr) {
    ...
    if (write_options.disableWAL) {
      return Status::NotSupported(
          "ZeroFlush: disableWAL not supported");
    }
    if (disable_memtable) {
      return Status::NotSupported(
          "ZeroFlush: disable_memtable not supported");
    }
    if (two_write_queues_) {
      return Status::NotSupported(
          "ZeroFlush: two_write_queues not supported");
    }
    if (immutable_db_options_.enable_pipelined_write) {
      return Status::NotSupported(
          "ZeroFlush: pipelined_write not supported");
    }
    if (seq_per_batch_) {
      return Status::NotSupported(
          "ZeroFlush: seq_per_batch not supported");
    }
    if (wbwi) {
      return Status::NotSupported(
          "ZeroFlush: WriteBatchWithIndex not supported");
    }
    if (write_options.protection_bytes_per_key > 0) {
      return Status::NotSupported(
          "ZeroFlush: protection_bytes_per_key > 0 not supported");
    }
  }
  WriteBatch* trace_batch =
  ...
  // M3.0 R3 修复：ZF 模式不使用原生 WAL，need_wal_sync 会让
  // PreprocessWrite 对 logs_ 调 PrepareForSync() 并等待 IsSyncing()
  // 清除，但 ZF 写路径跳过 WriteGroupToWAL，标记永远不清除 →
  // 下一个 sync 写阻塞在 wal_sync_cv_.Wait()（死锁）。ZF 的 fsync
  // 由 SyncTouchedPartitions 独立完成（见 M3_DESIGN.md §9）。
  if (zf_ctx_ != nullptr) {
  ...
  if (status.ok()) {
    ...
    if (!two_write_queues_) {
      if (status.ok() && !write_options.disableWAL && zf_ctx_ == nullptr) {
    ...
    if (zf_ctx_ == nullptr && wal_context.need_wal_sync) {
    ...
    if (status.ok()) {
      ...
      if (!parallel) {
        ...
        if (zf_ctx_ != nullptr) {
          // ---- ZeroFlush M2.1：封存触发点 ----
          // 写线程 leader（持 DB mutex）检查 ShouldSeal，若达阈值则同步
          // 封存全部 P 个分区 + 切换 MemTable。旧 mem 进 imm 等待物化。
          //
          // M2.3-2 修复：SealEpochAndSwitch → SwitchMemtable 内部会执行
          // mutex_.Unlock() / mutex_.Lock()（为 WAL 创建让出锁）。若调用时
          // 未持有 mutex，Unlock 释放不属于自己的锁，后续 Lock 获取到 mutex
          ...
          // 持锁窗口内执行）；触达分区由 zf_touched 带回，释放 mutex 后
          // 再 SyncTouchedPartitions（与原生 WriteToWAL 先 Unlock 再
          ...
          if (zf_ctx_->options().use_logger) {
            ROCKS_LOG_DEBUG(immutable_db_options_.info_log,
                            "ZF write: touched partitions=%zu sync=%d status=%s",
                            zf_touched.size(), write_options.sync ? 1 : 0,
...
// ---- ZeroFlush M2.1：封存+切表的公开包装 ----
// ZeroFlushContext::SealEpochAndSwitch 需要调 SwitchMemtable，但该函数
// 是 private。本函数在 DBImpl 内部，提供给 ctx 一个安全的入口。调用方
// （ctx）必须保证：(1) 持 DB mutex；(2) 是 write thread leader。
//
// M2 修复：SwitchMemtable 仅把 mutable mem 移入 imm_，但不会把 imm 加入
// flush 队列。若不显式 EnqueuePendingFlush，BG flush 永远不被调度，imm
// 堆积直到 write_controller 触发 kStopped，第二次封存后 WriteImpl 永久
// hang 在 DelayWrite。参考 IngestWriteBatchWithIndex / ScheduleFlushes
// 的做法，在切表后立即把 imm 标记 flush_requested 并 EnqueuePendingFlush。
Status DBImpl::ZfSwitchMemtable(ColumnFamilyData* cfd) {
  mutex_.AssertHeld();
  assert(cfd != nullptr);
  // SwitchMemtable 期望 4 参：cfd / context / new_imm=nullptr / last_seqno=0
  // 实际语义与 M2 触发的"无 WAL 重建"切表完全一致。
  WriteContext context;
  Status s = SwitchMemtable(cfd, &context, /*new_imm=*/nullptr, /*last_seqno=*/0);
  if (!s.ok()) {
    return s;
  }
  ...
  // 单 CF，与 atomic_flush 的多 CF 同步语义不耦合。
  //
  // 关键点：SwitchMemtable 内部会调 FlushRequested（设置 IsFlushPending），
  // 但不会 EnqueuePendingFlush。所以判定条件只看 queued_for_flush。
  // 跳过 atomic_flush 分支：ZeroFlush 当前只支持单 CF。
  if (!immutable_db_options_.atomic_flush && !cfd->queued_for_flush()) {
    cfd->imm()->FlushRequested();
    FlushRequest flush_req;
    GenerateFlushRequest({cfd}, FlushReason::kWriteBufferFull, &flush_req);
    EnqueuePendingFlush(flush_req);
