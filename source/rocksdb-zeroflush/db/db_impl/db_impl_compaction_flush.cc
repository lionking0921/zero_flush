...
Status DBImpl::FlushMemTable(ColumnFamilyData* cfd,
  ...
  // ---- ZeroFlush M2.1：取消 flush 抑制 ----
  // M1 时期 mem 永远不 flush（locator 依赖分区 WAL），但 M2 引入 epoch 引用
  // 链后，旧 mem 必须被 flush → SST，FlushJob 通过 MemTableIterator 取出
  // 每一项时已经过 zf_ctx_->ReadValue() 把 16B SlimLocator 解析为内联
  // value，所以 SST 自然生成内联 value 格式。mem 析构时 ~MemTable → ctx
  // → ReleaseEpoch → SealedFileCache 引用归零，封存文件进入 pending_unlink。
  ...
  autovector<uint64_t> memtable_ids_to_wait;
  {
    ...
    if (!cfd->mem()->IsEmpty() || !cached_recoverable_state_empty_.load() ||
      ...
      // ZeroFlush M2.1 补充（M3.3-27 修复）：手动 flush（CompactRange /
      // Flush）若直接把活跃 memtable 切 imm 而不封存 WAL 活跃代，已物化
      // 记录的 WAL 残留会在下次封存时被卷进新 epoch（A 侧混入旧 seq →
      // 融合归并 §7.4 的 seq 断言失败）。ZF 模式且 mem 非空时先走封存
      // （Freeze WAL + 切表 + epoch 关联），FlushJob 再按 ZF 路径物化。
      if (zf_ctx_ != nullptr && !cfd->mem()->IsEmpty()) {
        s = zf_ctx_->SealEpochAndSwitch(this, cfd);
      } else {
...
void DBImpl::MaybeScheduleFlushOrCompaction() {
  mutex_.AssertHeld();
  auto bg_job_limits_tmp = GetBGJobLimits();
  ROCKS_LOG_DEBUG(immutable_db_options_.info_log,
                  "MaybeScheduleFlushOrCompaction: unsched_flush=%d "
                  "bg_flush_sched=%d max_flushes=%d flush_pool=%d",
                  unscheduled_flushes_, bg_flush_scheduled_,
                  bg_job_limits_tmp.max_flushes,
