...
#include "zeroflush/zeroflush_db.h"  // ZeroFlush: shared_ptr<ZeroFlushContext>
...
class ColumnFamilyData {
 public:
  ...
  // ---- ZeroFlush：让 SwitchMemtable 产生的新 MemTable 继承 zf_ctx ----
  // 存储在 cfd 上，CreateNewMemtable 时设到新 mem，避免读到 16B locator 当 value。
  void SetZfCtx(const std::shared_ptr<zeroflush::ZeroFlushContext>& ctx) {
    zf_ctx_ = ctx;
  }
  const std::shared_ptr<zeroflush::ZeroFlushContext>& GetZfCtx() const {
    return zf_ctx_;
  }
 ...
 private:
  ...
  // ---- ZeroFlush：zf_ctx 由 cfd 持有，CreateNewMemtable 时设到新 mem ----
