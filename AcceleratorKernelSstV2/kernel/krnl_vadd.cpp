...
void decoder(ap_uint<32> *buf,  ap_uint<32> kv_sum, ap_uint<40> file_size, hls::stream<fifo_key_meta> &km_stream, hls::stream<fifo_value_slice> &value_stream)
{
    ...
    */
    ...
    // ---- 顺序窗预取状态：WAL 区窗缓冲（每口独立，非 static；仅主循环使用）----
    ap_uint<32> win[256];          // 当前窗内容：win[j] = WAL 字节区 (win_base+4j) 处 4B
    ap_uint<64> win_base = 0;      // 当前窗首字节地址（1024 对齐）
    ...
    for (ap_uint<32> i = 0; i < kv_sum; i++)
    {
        ...
        // ---------- 4) WAL 帧：物化 value（经顺序窗预取读取）----------
        // 帧读全部走 zfwin_*：miss 时把所在 1024B 窗整窗一次线性顺序读入 win[]，
        // 命中（相邻 slim 记录的帧落同一窗）则零 DRAM、片上取字节。帧内读取单调递增，
        // 跨窗界自动续载下一窗 —— WAL 区物理读从"逐记录随机细读"聚成整窗顺序突发。
        ap_uint<64> rec_addr = wal_offset;   // WAL 段在缓冲内从字节 0 起
        ap_uint<32> wkey_len = zfwin_u32(buf, win, win_base, win_valid, rec_addr + 8);
        ap_uint<32> val_len  = zfwin_u32(buf, win, win_base, win_valid, rec_addr + 12);
        ...
        for (ap_uint<7> w = 0; w < 64; w++)  // 64×16B = 1024B
        {
            ...
            if (base < val_len)
            {
                ...
                // 整 16B 经窗读取（固定位段拼装）；越窗界字节由 zfwin_ensure 逐字节续载
                ap_uint<128> t = zfwin_read16(buf, win, win_base, win_valid, a);
    ...
    if (good && K > 0)
    {
        ...
        for (ap_uint<16> f = 0; f < K && good; f++)
        {
            ...
            while (icur < i_end && good)
            {
                ...
                while (dcur < d_end && good)
                {
                    ap_uint<32> sh = zf_rd_varint32(buf, dcur);
                    ap_uint<32> us = zf_rd_varint32(buf, dcur);
                    ap_uint<32> vl = zf_rd_varint32(buf, dcur);
                    ...
                    // 重建 32B 内部键：前缀取 runkey[0..sh)，后缀来自本记录
                    const ap_uint<64> sstart = dcur;    // 后缀首字节
                    ...
                    {
                        nk[sh + j] = zf_ld_byte(buf, sstart + j);
                    }
                    dcur = sstart + us;
                    const ap_uint<64> vstart = dcur;    // value 首字节（块内联）
                    if (vstart + vl > d_end)
                    {
                    ...
                    {
                        ap_uint<64> wbase = (ap_uint<64>)w * 16;
                        if (wbase < vl)
                        {
                            ap_uint<128> t = zf_rd16(buf, vstart + wbase);
                            if (w < 32) sl0.c[w.range(4, 0)] = t;
                            else        sl1.c[w.range(4, 0)] = t;
                        }
                    }
