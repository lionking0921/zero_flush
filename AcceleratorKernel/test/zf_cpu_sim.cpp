...
// 字节流 -> ap_uint<32> 字数组（LE），尾部补 2 个 padding 字避免未对齐读越界
...
int main(int argc, char** argv) {
  ...
  std::vector<C> cases;
  if (argc > 1) {  // 允许外部传单组：seed ports base dup
  ...
  for (const auto& c : cases)
    if (RunCase(c.seed, c.ports, c.base, c.dup)) ++npass;
