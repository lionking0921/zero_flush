"""R44：50GB 终验——M4.6 L0 并行消费（subcompactions + l0_parallelism=8 多 job + ZF 恒并行）+ CURRENT 一致性修复 + kSkip 默认关。对比 R20（28.8K/247 停写/1.9h）。"""
"""R44：50GB 稳定性/完成度测试——最终配置 + 强化后台（针对 L0 消费）。
...
z.run_variant("R44", z.VARIANTS["R44"], z.HERE, force=True)
