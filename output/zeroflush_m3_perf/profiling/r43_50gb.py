"""R43：50GB 终验——M4.6 L0 并行消费（subcompactions + l0_parallelism=8 多 job + ZF 恒并行）+ CURRENT 一致性修复 + kSkip 默认关。对比 R20（28.8K/247 停写/1.9h）。"""
"""R43：50GB 稳定性/完成度测试——最终配置 + 强化后台（针对 L0 消费）。
...
z.VARIANTS["R43"] = dict(
    ...
    + ["--zf_routing=align_l1", "--zf_base_merge", "--zf_skip_batching=true", "--zf_partitions=16",
       "--subcompactions=16", "--cache_size=536870912",
       "--max_background_jobs=24"])
z.run_variant("R43", z.VARIANTS["R43"], z.HERE, force=True)
