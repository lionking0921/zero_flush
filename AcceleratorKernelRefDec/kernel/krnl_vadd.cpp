...
// 读 varint64（游标前进其所占字节数）—— B 侧 raw-SST 解码：block handle / footer handle
// 均为 LEB128 varint64；本档文件尺寸 < 2^35，实际 ≤5 字节，循环取到 10 字节上限兜底。
inline ap_uint<64> zf_rd_varint64(const ap_uint<32>* buf, ap_uint<64>& off)
{
    ap_uint<64> result = 0;
    for (ap_uint<4> i = 0; i < 10; i++)
    {
        ap_uint<8> b = zf_ld_byte(buf, off);
        off += 1;
        result |= (ap_uint<64>)(b & 0x7f) << (i * 7);
        if ((b & 0x80) == 0) break;
    }
    return result;
}
// 读 [addr, addr+16) 整 16B（DRAM 直读，未对齐安全，固定位段拼装，可综合）
inline ap_uint<128> zf_rd16(const ap_uint<32>* buf, ap_uint<64> addr)
{
    ap_uint<32> e0 = zf_ld_u32(buf, addr);
    ap_uint<32> e1 = zf_ld_u32(buf, addr + 4);
    ap_uint<32> e2 = zf_ld_u32(buf, addr + 8);
    ap_uint<32> e3 = zf_ld_u32(buf, addr + 12);
    ap_uint<128> t;
    t.range(31, 0) = e0;
    t.range(63, 32) = e1;
    t.range(95, 64) = e2;
...
/*
    ZeroFlush 片上突发原语 —— (AB-2 decoder 重构, 取代 zfwin 顺序窗)
    把一段连续 DRAM 字节 [start, start+span) 一次线性读入片上 ap_uint<32> 缓冲
    （DRAM 侧 = 连续突发，reference ap32to32_copy_datablock 同款），此后逐字节 /
    逐 16B 组访问全走片上 zfb_*（零 DRAM）。字节语义与 zf_* 直读逐字一致。
    约定：缓冲自 start 的 4B 对齐字起载 N 个字；内容首字节在片内偏移 head=start&3。
    zfb_* 的 rel 一律为相对内容首的字节偏移；调用方保证 head+rel+取宽落在已载字数内
    （且 ≤ 缓冲容量）。A 路 value 段与 B 路整 data block 共用同一对原语。
*/
// 连续线性读入：buff[0..n_words) = DRAM [(start>>2) .. +n_words)（start 任意未对齐）
inline void zf_burst_load(const ap_uint<32>* buf, ap_uint<32> buff[], ap_uint<64> start, ap_uint<16> n_words)
{
    ap_uint<64> base = start >> 2;
    for (ap_uint<16> j = 0; j < n_words; j++)
    {
        buff[j] = buf[base + (ap_uint<64>)j];
    }
}
// 片上读 1 字节（head+rel = 片内字节偏移，LE）
inline ap_uint<8> zfb_byte(const ap_uint<32> buff[], ap_uint<2> head, ap_uint<16> rel)
{
    ap_uint<16> a = (ap_uint<16>)head + rel;
    ap_uint<32> wd = buff[a.range(15, 2)];
    switch (a.range(1, 0))
    {
    case 0:  return wd.range(7, 0);
    case 1:  return wd.range(15, 8);
...
// 片上读 4 字节（LE）
inline ap_uint<32> zfb_u32(const ap_uint<32> buff[], ap_uint<2> head, ap_uint<16> rel)
{
    ap_uint<16> a = (ap_uint<16>)head + rel;
    ap_uint<32> w0 = buff[a.range(15, 2)];
    ap_uint<32> w1 = buff[a.range(15, 2) + 1];
    switch (a.range(1, 0))
    {
    case 0:  return w0;
    case 1:  return (w0 >> 8) | (w1 << 24);
...
// 片上读 varint32（rel 前进所占字节数）
inline ap_uint<32> zfb_varint32(const ap_uint<32> buff[], ap_uint<2> head, ap_uint<16>& rel)
{
    ap_uint<32> result = 0;
    for (ap_uint<3> i = 0; i < 5; i++)
    {
        ap_uint<8> b = zfb_byte(buff, head, rel);
        rel += 1;
        result |= (ap_uint<32>)(b & 0x7f) << (i * 7);
        if ((b & 0x80) == 0) break;
    }
...
// 片上读 [rel, rel+16) 整 16B（固定 5 字 lane 位段拼装，同 Get_value_for_decoder 的可综合形态）
inline ap_uint<128> zfb_rd16(const ap_uint<32> buff[], ap_uint<2> head, ap_uint<16> rel)
{
    ap_uint<16> a = (ap_uint<16>)head + rel;
    ap_uint<16> si = a.range(15, 2);
    ap_uint<128> t;
    switch (a.range(1, 0))
    {
    case 0:
        t.range(31, 0)   = buff[si];
        t.range(63, 32)  = buff[si + 1];
        t.range(95, 64)  = buff[si + 2];
        t.range(127, 96) = buff[si + 3];
        break;
    case 1:
        t.range(23, 0)   = buff[si].range(31, 8);
        t.range(55, 24)  = buff[si + 1];
        t.range(87, 56)  = buff[si + 2];
        t.range(119, 88) = buff[si + 3];
        t.range(127, 120) = buff[si + 4].range(7, 0);
        break;
    case 2:
        t.range(15, 0)   = buff[si].range(31, 16);
        t.range(47, 16)  = buff[si + 1];
        t.range(79, 48)  = buff[si + 2];
        t.range(111, 80) = buff[si + 3];
        t.range(127, 112) = buff[si + 4].range(15, 0);
        break;
    default:
        t.range(7, 0)    = buff[si].range(31, 24);
        t.range(39, 8)   = buff[si + 1];
        t.range(71, 40)  = buff[si + 2];
        t.range(103, 72) = buff[si + 3];
        t.range(127, 104) = buff[si + 4].range(23, 0);
    }
    return t;
}
...
void decoder(ap_uint<32> *buf,  ap_uint<32> kv_sum, ap_uint<40> file_size, hls::stream<fifo_key_meta> &km_stream, hls::stream<fifo_value_slice> &value_stream)
{
    ...
    */
    ...
    // ---- A 路 value 片上缓冲（AB-2 burst 重构）：每记录整帧 value 段 burst 进此再切片 ----
    ap_uint<32> value_buf[512];    // 512×u32 = 2KB ≫ 值上限 1024B + 未对齐字 + 16B 组尾
    ...
    for (ap_uint<32> i = 0; i < kv_sum; i++)
    {
        ...
        // ---------- 4) WAL 帧：物化 value（整帧 value 段一次 burst 进片上）----------
        // 帧头 @+8 wkey_len u32 / @+12 val_len u32 一次性 DRAM 直读（8B 连续、未对齐安全）；
        // value 字节区间 [vbase, vbase+val_len)（val_len ≤1024，删除标记=0）一次 zf_burst_load
        // 连续读入 value_buf，切片从片上取 —— 取代里程碑 2 的逐字节 zfwin 窗（每记录 ~1 整窗
        // 重载 + 16B 摊 16 次 ensure）。
        ap_uint<64> rec_addr = wal_offset;   // WAL 段在缓冲内从字节 0 起
        ap_uint<64> hdr8 = zf_rd_u64(buf, rec_addr + 8);   // [8..16)：wkey_len | val_len（LE）
        ap_uint<32> wkey_len = hdr8.range(31, 0);
        ap_uint<32> val_len  = hdr8.range(63, 32);
        // 帧首 @0：magic u32；@4 cf_id u16；@6 type u8；@7 flags u8；@16 seq u64
        ap_uint<64> vbase = rec_addr + 24 + wkey_len;

        // 载字：盖到值末的 16B 组界（切片整组从片上读，不越过已载区）
        ap_uint<32> vsz = (val_len > (ap_uint<32>)VALUE_LENGTH) ? (ap_uint<32>)VALUE_LENGTH : val_len;
        ap_uint<2>  vhead = vbase.range(1, 0);
        if (vsz != 0)
        {
            ap_uint<32> span = ((vsz + 15) >> 4) << 4;              // 末组界（≤1024）
            ap_uint<32> nw = (span + (ap_uint<32>)vhead + 3) >> 2;  // 载字数 ≤ 257
            zf_burst_load(buf, value_buf, vbase, (ap_uint<16>)nw);
        }
        ...
        {
            ap_uint<16> base = (ap_uint<16>)w * 16;
            if ((ap_uint<64>)base < val_len)
            {
                // 整 16B 从片上 value_buf 取（固定位段拼装）；vsz==0 → 无组解码（全零）
                ap_uint<128> t = zfb_rd16(value_buf, vhead, base);
...
/*
    ============================================================
    ZeroFlush B 侧 raw-SST 解码器 decoder_sst —— (AB-1)
    ============================================================
    ...
    每文件解码（顺序流式，不做随机 seek）：
      ...
      3) 对每个 data block：内容整块顺序解码，按 block record + restart 前缀
         重建每条记录的完整键；键长必须 == KEY_LENGTH(32)（24B user + 8B footer，
         §14.6 档），value 长 ≤ VALUE_LENGTH(1024)，vlen=0 = 删除标记；
      4) 输出协议与 A decoder 逐字一致：流头 SIGNAL → 每记录 1×km（keystring
         为 32B 原始内部键、自然字节序）+ 固定 2×512B value 切片（尾部零填充）
         → 流尾 MAX。空链 = 只发 SIGNAL+MAX。

    结构错误（magic/version/checksum/越界/键长/值长）→ printf 诊断 + 提前结束
    该文件解码（外层仍发 MAX），sim 侧以"总记录数与预言机比对"暴露 —— 绝不静默。
    本里程碑 B 输入文件由本 kernel 自己的 full-version encoder 预产并经
    zf_seal_check 封口锚定"确为引擎可读 §14.6 文件"，故按上述硬约束解析即可。
*/
#define ZF_SST_CHAIN_MAX_FILES 64u   // B 链描述表最多文件数（坏输入死循环保护）

void decoder_sst(ap_uint<32>* buf, ap_uint<40> file_size, hls::stream<fifo_key_meta>& km_stream, hls::stream<fifo_value_slice>& value_stream)
{
    // 与 A decoder 相同的流头 SIGNAL
    {
        fifo_key_meta SIGNAL;
        set_signal_key(SIGNAL.key);
        km_stream.write(SIGNAL);
    ...
    // ---- 链描述表 ----
    bool good = true;
    // ---- 片上 data-block 缓冲（AB-2 burst 重构）：单块整块 burst 进此再解，见 3a) ----
    // 1304×u32 ≈ 5216B：块内容上限 5200B（reference bram_buffer 1300 字同量级）+ 未对齐/组尾松弛
    ap_uint<32> blk[1304];
    ap_uint<64> chain_bytes = file_size;
    ap_uint<64> K = 0;
    if (chain_bytes >= 8)
    {
        K = zf_rd_u64(buf, 0);
        if (K > ZF_SST_CHAIN_MAX_FILES)
        {
            printf("decoder_sst: chain file count %llu > %d, abort chain\n",
                   (unsigned long long)K.to_uint64(), (int)ZF_SST_CHAIN_MAX_FILES);
            good = false;
        }
    }
    ...
    {
        ap_uint<64> desc = 8;
        for (ap_uint<16> f = 0; f < K && good; f++)
        {
            if (desc + 16 > chain_bytes)
            {
                printf("decoder_sst: descriptor %d out of range\n", (int)f);
                good = false;
                break;
            }
            ap_uint<64> foff = zf_rd_u64(buf, desc);
            ap_uint<64> fsz  = zf_rd_u64(buf, desc + 8);
            desc += 16;
            if (fsz == 0)
            {
                continue;  // 空文件条目 → 无记录
            }
            if (foff < 8 || foff + fsz > chain_bytes)
            {
                printf("decoder_sst: file %d region [%llu,%llu) out of chain %llu\n",
                       (int)f, (unsigned long long)foff.to_uint64(),
                       (unsigned long long)fsz.to_uint64(),
                       (unsigned long long)chain_bytes.to_uint64());
                good = false;
                break;
            }
            ...
            if (fsz < 53)
            {
                printf("decoder_sst: file %d too small (%llu < 53)\n", (int)f,
                       (unsigned long long)fsz.to_uint64());
                good = false;
                break;
            }
            ap_uint<64> magic = zf_rd_u64(buf, FE - 8);
            ap_uint<32> ver   = zf_ld_u32(buf, FE - 12);
            ap_uint<8>  chk   = zf_ld_byte(buf, FE - 53);
            if (magic != 0x88e241b785f4cff7ULL)
            {
                printf("decoder_sst: file %d bad magic %016llx\n", (int)f,
                       (unsigned long long)magic.to_uint64());
                good = false;
                break;
            }
            if (ver != 2)
            {
                printf("decoder_sst: file %d unsupported format_version %u\n",
                       (int)f, (unsigned)ver.to_uint());
                good = false;
                break;
            }
            ...
            // 越过 chk_type：footer 起点 = FE-53，part2 起点 = FE-52
            ap_uint<64> fcur = FE - 52;
            ap_uint<64> meta_off = zf_rd_varint64(buf, fcur);  // 丢弃
            ap_uint<64> meta_sz  = zf_rd_varint64(buf, fcur);  // 丢弃
            ap_uint<64> idx_off  = zf_rd_varint64(buf, fcur);
            ap_uint<64> idx_sz   = zf_rd_varint64(buf, fcur);
            // index content（不含 trailer）须整体落在文件内
            if (idx_off + idx_sz > fsz || idx_sz < 4)
            {
                printf("decoder_sst: file %d bad index handle off=%llu sz=%llu (fsz=%llu)\n",
                       (int)f, (unsigned long long)idx_off.to_uint64(),
                       (unsigned long long)idx_sz.to_uint64(),
                       (unsigned long long)fsz.to_uint64());
                good = false;
                break;
            }
            const ap_uint<64> IDX = FB + idx_off;  // index content 绝对偏移

            // ---- 2) index 块：num_restarts 在内容末 4B，records 到 restart 数组前 ----
            ap_uint<32> i_nr = zf_ld_u32(buf, IDX + idx_sz - 4);
            if (i_nr > 4096 || (ap_uint<64>)(i_nr)*4 + 4 > idx_sz)
            {
                printf("decoder_sst: file %d bad index num_restarts %u\n", (int)f,
                       (unsigned)i_nr.to_uint());
                good = false;
                break;
            }
            ap_uint<64> i_end = IDX + idx_sz - ((ap_uint<64>)i_nr * 4 + 4);
            ap_uint<64> icur = IDX;

            // ---- 3) 顺序遍历 index 记录，逐 data 块解码 ----
            ap_uint<32> entry_cnt = 0;
            while (icur < i_end && good)
            {
                ap_uint<32> sh = zf_rd_varint32(buf, icur);
                ap_uint<32> us = zf_rd_varint32(buf, icur);
                ap_uint<32> vl = zf_rd_varint32(buf, icur);
                if (us == 0 || sh + us > KEY_LENGTH || icur + us > i_end)
                {
                    printf("decoder_sst: file %d bad index entry key sh=%u us=%u\n",
                           (int)f, (unsigned)sh.to_uint(), (unsigned)us.to_uint());
                    good = false;
                    break;
                }
                icur += us;                    // 跳过后缀字节（index 键可短于 32）
                if (vl == 0 || vl > 24 || icur + vl > i_end)
                {
                    printf("decoder_sst: file %d bad index entry value len %u\n",
                           (int)f, (unsigned)vl.to_uint());
                    good = false;
                    break;
                }
                // value = BlockHandle：varint64(off) + varint64(size)
                ap_uint<64> hcur = icur;
                ap_uint<64> doff = zf_rd_varint64(buf, hcur);
                ap_uint<64> dsz  = zf_rd_varint64(buf, hcur);
                if (hcur - icur != vl)          // value 必须恰好是该 handle 的 varint 字节
                {
                    printf("decoder_sst: file %d index handle len mismatch\n", (int)f);
                    good = false;
                    break;
                }
                icur += vl;

                // ---- 3a) 解码对应 data block（内容在 [FB+doff, +dsz)，5B trailer 在后）----
                // AB-2 重构：整块内容一次连续 burst 进 blk（reference ap32to32_copy_datablock
                // 同款），此后记录解码全走片上 zfb_* —— 取代原"每 16B/逐字节散射 DRAM 读"。
                if (doff + dsz + 5 > fsz || dsz < 4)
                {
                    printf("decoder_sst: file %d bad data handle off=%llu sz=%llu\n",
                           (int)f, (unsigned long long)doff.to_uint64(),
                           (unsigned long long)dsz.to_uint64());
                    good = false;
                    break;
                }
                const ap_uint<64> DB = FB + doff;
                ap_uint<32> d_nr = zf_ld_u32(buf, DB + dsz - 4);
                if (d_nr > 4096 || (ap_uint<64>)d_nr * 4 + 4 > dsz)
                {
                    printf("decoder_sst: file %d bad data block num_restarts\n", (int)f);
                    good = false;
                    break;
                }
                ap_uint<64> d_end = DB + dsz - ((ap_uint<64>)d_nr * 4 + 4);
                // 守卫：块内容 ≤ 5200B（blk 缓冲上限，reference bram_buffer 1300 字同量级）；
                // 引擎块 ~4KB 关块绝不触发；超限 = 结构错误（诊断后结束本文件）。
                if (dsz > 5200)
                {
                    printf("decoder_sst: file %d data block too large %u (>5200)\n",
                           (int)f, (unsigned)dsz.to_uint());
                    good = false;
                    break;
                }
                const ap_uint<2>  bhead = DB.range(1, 0);      // 块首字内字节偏移
                ap_uint<16> bnw = (ap_uint<16>)((((ap_uint<32>)bhead + dsz) + 3) >> 2);  // ≤ 1301
                zf_burst_load(buf, blk, DB, bnw);
                ap_uint<16> dend = (ap_uint<16>)(d_end - DB);  // 记录区相对长度（<5200）
                ap_uint<16> drel = 0;                          // 记录游标（相对 DB）

                ap_uint<8> runkey[KEY_LENGTH];          // 本 data 块 running key（块间独立）
                for (ap_uint<6> z = 0; z < KEY_LENGTH; z++) { runkey[z] = 0; }

                while (drel < dend && good)
                {
                    ap_uint<32> sh = zfb_varint32(blk, bhead, drel);
                    ap_uint<32> us = zfb_varint32(blk, bhead, drel);
                    ap_uint<32> vl = zfb_varint32(blk, bhead, drel);
                    if (us == 0 || sh + us != KEY_LENGTH || sh > KEY_LENGTH)
                    {
                        printf("decoder_sst: file %d bad data record key sh=%u us=%u\n",
                               (int)f, (unsigned)sh.to_uint(), (unsigned)us.to_uint());
                        good = false;
                        break;
                    }
                    if (vl > VALUE_LENGTH)
                    {
                        printf("decoder_sst: file %d value len %u > %d\n", (int)f,
                               (unsigned)vl.to_uint(), (int)VALUE_LENGTH);
                        good = false;
                        break;
                    }
                    // 重建 32B 内部键：前缀取 runkey[0..sh)，后缀来自本记录（片上字节）
                    const ap_uint<16> srel = drel;      // 后缀首（相对 DB）
                    ap_uint<8> nk[KEY_LENGTH];
                    for (ap_uint<6> j = 0; j < KEY_LENGTH; j++) { nk[j] = 0; }
                    for (ap_uint<6> j = 0; j < sh; j++)       { nk[j] = runkey[j]; }
                    for (ap_uint<6> j = 0; j < us; j++)
                    {
                        nk[sh + j] = zfb_byte(blk, bhead, (ap_uint<16>)(srel + (ap_uint<16>)j));
                    }
                    drel = srel + (ap_uint<16>)us;
                    const ap_uint<16> vrel = drel;      // value 首（块内联）
                    if ((ap_uint<16>)(vrel + (ap_uint<16>)vl) > dend)
                    {
                        printf("decoder_sst: file %d data record value overruns block\n",
                               (int)f);
                        good = false;
                        break;
                    }
                    drel = vrel + (ap_uint<16>)vl;

                    // keystring：c[0]=ik[0..16)，c[1]=ik[16..32)，自然字节序
                    keystring key;
                    key.length = KEY_LENGTH;
                    {
                        ap_uint<128> w0 = 0;
                        ap_uint<128> w1 = 0;
                        for (ap_uint<5> bj = 0; bj < 16; bj++)
                        {
                    #pragma HLS UNROLL
                            w0.range(bj * 8 + 7, bj * 8) = nk[bj];
                            w1.range(bj * 8 + 7, bj * 8) = nk[bj + 16];
                        }
                        key.c[0] = w0;
                        key.c[1] = w1;
                    }

                    // 先发 km（与 A decoder 一致：km 先于 value 切片）
                    fifo_key_meta km;
                    km.key = key;
                    km.value_length = vl.range(VALUE_LENGTH_BITWIDTH - 1, 0);
                    km_stream.write(km);

                    // 固定 2×512B 切片；value 字节 [0,vl) 落在 1024B 窗口前部
                    fifo_value_slice sl0;
                    fifo_value_slice sl1;
                    for (ap_uint<6> k = 0; k < 32; k++) { sl0.c[k] = 0; sl1.c[k] = 0; }
                    for (ap_uint<7> w = 0; w < 64; w++)
                    {
                        ap_uint<16> wbase = (ap_uint<16>)w * 16;
                        if ((ap_uint<64>)wbase < vl)
                        {
                            ap_uint<128> t = zfb_rd16(blk, bhead, (ap_uint<16>)(vrel + wbase));
                            if (w < 32) sl0.c[w.range(4, 0)] = t;
                            else        sl1.c[w.range(4, 0)] = t;
                        }
                    }
                    ...
                    for (ap_uint<6> j = 0; j < KEY_LENGTH; j++) { runkey[j] = nk[j]; }
                }   // end 该 data block 记录循环

                entry_cnt++;
                if (entry_cnt > 100000)
                {
                    printf("decoder_sst: file %d too many index entries\n", (int)f);
                    good = false;
                    break;
                }
            }   // end index 记录循环
...
// ===== 里程碑 3：kernel CRC-32C 表 + 块辅助（v2 + kCRC32c §14.6 档位）=====
#include "zf_crc32c_kernel.inc"
...
{
    // (里程碑 3) 索引 entry 升到标准 block record（v2 单级索引，index_key_is_user_key=false）：
    //   [shared varint][unshared varint][value_length varint][key][value]
    // 修复两缺陷：1) 键不再截断为低 8B —— 写完整 32B internal key（24B user + 8B seq/type 尾）；
    //             2) 补齐缺失的第三个 varint value_length（引擎 BlockBasedTable 逐 record 解析必需）。
    // shared 恒 0（整键、不做前缀压缩），unshared=32；重启间隔仍由调用方打点，打点偏移 = 记录首
    // 字节（合法）。value = BlockHandle = [varint64 offset][varint64 size]，共 value_length 字节。
    // shared varint（0 → 1 字节）
    ap8to128_encoder_index_block(0, sst_pointer, index);   index += 1;
    // unshared varint（KEY_LENGTH=32 < 128 → 1 字节）
    ap8to128_encoder_index_block(KEY_LENGTH, sst_pointer, index);   index += 1;
    // value_length varint（≤ 8 < 128 → 1 字节）
    ap8to128_encoder_index_block(value_length, sst_pointer, index); index += 1;
    // key：完整 32B internal key（key.c[i] 为 16B 自然字节镜像），逐字节经字节游标写 DRAM
    for (ap_uint<6> j = 0; j < KEY_LENGTH; j++)
    {
        ap_uint<4> cw   = j.range(4, 4);   // 0..1 → key.c[0..1]
        ap_uint<4> lane = j.range(3, 0);
        ap8to128_encoder_index_block(zf_crc_word_byte(key.c[cw], lane), sst_pointer, index);
        index += 1;
    }
    // value：BlockHandle varint 低 value_length 字节（value 为 [offset varint][size varint] 的低位拼装）
    ap_uint<8> vsh = 0;
    for (ap_uint<5> vi = 0; vi < value_length; vi++)
    {
        ap_uint<64> vs = value >> vsh;    // 动态移位（≤56）可综合
        ap8to128_encoder_index_block(vs.range(7, 0), sst_pointer, index);
        index += 1;
        vsh += 8;
    }
}
            ...
            )
{
    ...
    // Post-loop tail file-close (mode=2 t7 fix, a69cf31-routable layout):
    // the in-loop close above sits in the else(valid) branch and fires on file
    // rollover or at i==kv_sum-1.  If the stream's LAST record(s) are same-user-key
    // suppressed (keep_all_versions==0, e.g. mode=2 kMergeBase tail), that close
    // never runs and the final open file would be left unflushed/unclosed
    // (4cc31bb "fix compaction kernel (SW)" bug).  Reproduced here once, AFTER
    // the loop, guarded by "an open file still has entries" -- keeps the encoder
    // main-loop schedule identical to the routable a69cf31 layout (@300MHz E-1).
...
/*
    ============================================================
    端口解码分派器 —— (AB-2)
    kind = 0：A staged（封存 WAL 段字节 + slim 条目区）→ decoder
    kind = 1：B raw-SST 链（[u64 K][K×{off,sz}][文件字节...]）→ decoder_sst
    输出流协议两者逐字一致，故 merge / encoder 不感知端口类别。
    注意：krnl_vadd 的 DATAFLOW 里以"一个进程（本函数）"出现；分支在进程内部
    顺序展开，kind 来自 host_data —— 两分支皆可综合。
    ============================================================
*/
inline void decode_port(ap_uint<32>* buf, ap_uint<64> kv, ap_uint<64> file_size, ap_uint<64> kind,
                        hls::stream<fifo_key_meta>& km_stream,
                        hls::stream<fifo_value_slice>& value_stream)
{
    if (kind == 1)
    {
        decoder_sst(buf, file_size.range(39, 0), km_stream, value_stream);
    }
    else
    {
        decoder(buf, kv.range(31, 0), file_size.range(39, 0), km_stream, value_stream);
    }
}

/*
    ============================================================
    ZeroFlush 内核顶层 —— 4 口 A staged / B raw-SST 链 混合输入，
    dataflow：4×decode_port → merge → encoder（全版本 / aux-sort 去重两档）
    ============================================================
*/
...
#pragma HLS array_partition variable=port_kind type=complete dim=1
        ...
        // host_data[14] 保留未用
        mode      = host_data[15];
        port_kind[0] = host_data[16];
        port_kind[1] = host_data[17];
        port_kind[2] = host_data[18];
        port_kind[3] = host_data[19];
        // host_data[20..23] 保留未用
...
#pragma HLS array_partition variable=index_block_limit type=complete dim=1
        ...
        decode_port(sst_input0, input_kv[0], size[0], port_kind[0],
                decoder_km_stream[0], encoder_value_stream[0]);
        decode_port(sst_input1, input_kv[1], size[1], port_kind[1],
                decoder_km_stream[1], encoder_value_stream[1]);
        decode_port(sst_input2, input_kv[2], size[2], port_kind[2],
        ...
        // mode 归一为 encoder 的 keep_all_versions 标量：只有 A+B 全版本（mode=1）关闭
        // 同 user-key 连续版本抑制；M3 aux-sort 去重（mode=0）与 kMergeBase A∪B 真重写归并
        // （mode=2，每 user 键只留最新、最新为删除则保 tombstone）都走 keep_all_versions=0
        // （保留首见 = seq 降序流上的最新版）。host 命名契约见 krnl_host.h。
        ap_uint<32> keep_all_versions = (mode.range(31, 0) == 1) ? (ap_uint<32>)1 : (ap_uint<32>)0;
